...
export interface MortgageParams {
	...
	/** Первый взнос */
	downPayment?: number
	/** Досрочное погашение каждый месяц (уменьшение тела). Игнорируется, если задан getMonthlyExtra. */
	monthlyExtra?: number
	/** Досрочка по месяцам (месяц 1-based). Если задано, используется вместо monthlyExtra. */
	getMonthlyExtra?: (month: number) => number
}
...
export function buildSchedule(params: MortgageParams): {
	...
	while (balance > 0 && month < 600) {
		...
		month++
		const extraForMonth = getMonthlyExtra ? getMonthlyExtra(month) : monthlyExtra
		...
		let extra = Math.min(extraForMonth, balance - toPrincipal)
		if (extra < 0) extra = 0
